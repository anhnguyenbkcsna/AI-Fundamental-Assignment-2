...
class Othello:
    ...
    def play(self):
        ...
        if is_player_move == False:
            ...
            is_player_move = True
            player_start_time = time.time()
            self.grid.insertToken(self.grid.gridLogic, self.currentPlayer, selected_move[1], selected_move[0])
            swappableTiles = self.grid.swappableTiles(selected_move[1], selected_move[0], self.grid.gridLogic, self.currentPlayer)
            for tile in swappableTiles:
                self.grid.animateTransitions(tile, self.currentPlayer)
                self.grid.gridLogic[tile[0]][tile[1]] *= -1
            self.currentPlayer *= -1
        ...
        for event in pygame.event.get(): # quit game
            ...
            if event.type == pygame.MOUSEBUTTONUP and clicked == True:
                ...
                if square in possible_move:
                    ...
                    self.grid.insertToken(self.grid.gridLogic, self.currentPlayer, square[1], square[0])
                    swappableTiles = self.grid.swappableTiles(square[1], square[0], self.grid.gridLogic, self.currentPlayer)
                    for tile in swappableTiles:
                        self.grid.animateTransitions(tile, self.currentPlayer)
                        self.grid.gridLogic[tile[0]][tile[1]] *= -1
                    self.currentPlayer *= -1
